...
#define IP_DEFAULT_HDR_LEN 20     // IPv4 默认头部长度
#define ETHERNET_MTU 1500         // 以太网 MTU
...
void ip_fragment_out(buf_t *buf, uint8_t *ip, net_protocol_t protocol, int id, uint16_t offset, int mf) {
    // Step1: 增加头部缓存空间
    if (buf_add_header(buf, IP_DEFAULT_HDR_LEN) < 0) {
        return;  // 添加头部失败，丢弃该包
    }
    ...
    ip_hdr->hdr_len = IP_DEFAULT_HDR_LEN / 4;  // 单位是 4 字节
    ...
    // Step3: 计算并填写校验和
    ip_hdr->hdr_checksum16 = 0;
    ip_hdr->hdr_checksum16 = checksum16((uint16_t *)ip_hdr, IP_DEFAULT_HDR_LEN);
...
void ip_out(buf_t *buf, uint8_t *ip, net_protocol_t protocol) {
    ...
    const int MAX_PAYLOAD = ETHERNET_MTU - IP_DEFAULT_HDR_LEN;
